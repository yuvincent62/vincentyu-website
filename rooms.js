/*    function defineAndDisplayRoomMarkers(school, floor,parameter) {
      ...
      let valueArray = valueArrayIn.map(num => parseFloat(num.toFixed(2)));
      ...
      function defineAndDisplayRoomMarkers(school, floor, parameter) {
        ...
        if (parameter === 'CO2') {
          ...
          suffix = 'µg/m\u00B3';
        } else if (parameter === 'Dew Point' || parameter === 'Temperature') {
          suffix = '\u00B0F';
        ...
        function addModifiedRoomMarker(room, color, width, height, lat, lng, code, value) {
          addRoomMarker(room, color, width, height, lat, lng, code, value + suffix);
        }
